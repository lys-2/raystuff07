...
struct trace { bool is_hit; double a, b, t; struct v3 o, d, l, n; };
...
struct user { char name[16], pw[16], progress, possess; int score; };
struct actor {
    char name[16], type[8], parent, lvl, state[8],
        tags[8], user, controlled, party[8], path, gen, age, height, weight,
        deep, thick, cover, rain, wet, light;
    bool spawned; short stack, hp; int cost, x, y;
    struct v3 l, r, s;

};
struct actor ad = { "dummy", "item", 0, 1, 1, .hp = 10 };
...
struct trace raysp(struct v3 so, double rd, struct v3 ro, struct v3 dir) {
    ...
    h = sqrt(h);
    if (h > 0.0) {
        res.is_hit = true; res.t = min(-b - h, -b + h);
        res.l = addv(ro, muls(dir, res.t));
        res.n = norm(addv(muls(dir, res.t), oc));
    }
...
void clear() {
    for (ln i = 0; i < 496 * 216; i++) {
        frame.pixels[i].b = 0;
        frame.pixels[i].g = 0;
        frame.pixels[i].r /= 1.1;
    }
}
void point(double x, double y, double z, char c) {
    if ((x >= 0 && x < 496) && (y >= 0 && y < 216)) {
        frame.pixels[(ln)(round(x) + (round(y) * 496))].b = 0;
        frame.pixels[(ln)(round(x) + (round(y) * 496))].r = 0;
        frame.pixels[(ln)(round(x) + (round(y) * 496))].g = c;
      //  p += 1;
    }
...
void ring(struct v3 a, db r) {
    for (int pixel = 0; pixel < 5*r; pixel++) {
...
void console() {
 ...
 enum mat { DIFF, SPEC, REFR };  // material types, used in radiance()
 struct Sphere { double r; vc pos, em, color; enum mat m; };
 struct Sphere spheres[] = {//Scene: radius, position, emission, color, material
  {1e5, { 1e5 + 1,40.8,81.6 }, {0} , {.0,.99,.0}, DIFF},//Left
   ...
   //splay(); splay();
    ...
    // BeginPaint(window_handle, &paint);
     ...
     for (ln i = 0; i < 496 * 217; i++) {
         ...
         frame.pixels[i].g = 111*x/ 496;
        // frame.pixels[i].r = 0;
        frame.pixels[i].g = 111 * y / f/.1;
...
LRESULT CALLBACK WindowProcessMessage(HWND window_handle,
    ...
    case WM_MOUSEMOVE: { 
        ...
        struct trace hit = raysp(
            ...
            33,
            (struct v3) {
            LOWORD(lParam), 216 - HIWORD(lParam)
        },
            (struct v3) {
            0, 0, 1
        }
        ...
        if (hit.is_hit) { 
            ...
            printf_s("S %f, %f ", hit.l.x, hit.l.y);
            SetConsoleTextAttribute(hConsole, 15);
        };
        struct actor a;
        a.l.x = LOWORD(lParam);
